...
### Connection points straight from the STEP CAD (no ML, exact)

When a `.stp` is available, the connection openings are real CAD features
(`CYLINDRICAL_SURFACE`). `step_openings.py` reads them via gmsh — exact centres +
insert axes, in the mesh frame, **no model needed**:
...
manually with `--rmin/--rmax` after inspecting `--list`. Only circular /
cylindrical openings are caught, and one unreadable STEP no longer aborts a batch.
...
`--keep-duplicates` to keep them. The insert direction now points out of the
nearer face for blind holes/recesses. `--drop-corner FRAC` is an EXPERIMENTAL,
